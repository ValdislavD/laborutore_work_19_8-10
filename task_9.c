...
void createAthletesFile(const char* filename) {
    ...
    fwrite(&count, sizeof(int), 1, file); // Записываем количество спортсменов в файл

    for (int i = 0; i < count; ++i) {
        fwrite(&athletes[i], sizeof(Athlete), 1, file); // Записываем информацию о спортсмене в файл
    }
...
Athlete* readAthletes(const char* filename, int* count) {
    ...
    fread(count, sizeof(int), 1, file); // Читаем количество спортсменов из файла
    Athlete *athletes = (Athlete*)malloc(*count * sizeof(Athlete));

    for (int i = 0; i < *count; ++i) {
        fread(&athletes[i], sizeof(Athlete), 1, file); // Читаем информацию о спортсмене из файла
    }
    ...
    for (int i = 0; i < count - 1; ++i) {
        for (int j = 0; j < count - i - 1; ++j) {
            if (athletes[j].bestResult < athletes[j + 1].bestResult) {
                // Меняем местами спортсменов, если их результаты не в том порядке
...
void writeTeamToFile(Athlete* team, int count, const char* filename) {
    ...
    for (int i = 0; i < count; ++i) {
        fwrite(&team[i], sizeof(Athlete), 1, file); // Записываем информацию о спортсмене в файл
    }
...
void checkFileContent(const char* filename, Athlete* expectedAthletes, int expectedCount) {
    int count;
